...
export async function GetOidc(path, oidcToken) {
    return new Promise( (resolve, reject) => {
        axios({
            ...
            const schoolid = resp.data.schoolid;
            const sub = resp.data.sub;
            const titles = resp.data.titles[0].titles[0];
            const classtitle = resp.data.classinfo[0].classtitle;
            const grade = resp.data.classinfo[0].grade;
            const classno = resp.data.classinfo[0].classno;
            const eduinfo = { "schoolid": schoolid, "sub":sub, "titles":titles, 
...
export async function PostOidc(token_ep, code, redir_uri) {
    ...
    return new Promise( (resolve, reject) => {
        axios({
            ...
            const data = {access_token: response.data.access_token};
            resolve(data);
        })
        .catch(function (error) {
            console.log(error);
